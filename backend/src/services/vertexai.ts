...
import type { DetectedProduct, EvidencePack, Claim, EcoScore } from '../types/index.js';
...
export async function analyzeImageWithGemini(imageBase64: string): Promise<DetectedProduct> {
...
): Promise<{ claims: Claim[]; overallConfidence: number }> {
    const vertex = getVertexAI();
    const model = vertex.getGenerativeModel({ model: 'gemini-1.5-pro' });
...
export async function generateEcoScoreExplanation(
    product: DetectedProduct,
    ecoScore: Omit<EcoScore, 'summary' | 'rationaleBullets'>,
    claims: Claim[]
): Promise<{ summary: string; rationaleBullets: string[] }> {
    const vertex = getVertexAI();
    const model = vertex.getGenerativeModel({ model: 'gemini-1.5-pro' });
...
export async function generateGemmaTips(
    product: DetectedProduct,
    materials: string[]
): Promise<string[]> {
    const vertex = getVertexAI();
