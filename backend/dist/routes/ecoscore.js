...
function calculateSourcingScore(product, claims) {
    let score = 20; // Start at midpoint
    // Check for high-risk materials
    const hasHighRisk = product.materialsUsed.some(m => HIGH_RISK_MATERIALS.some(hr => m.toLowerCase().includes(hr.toLowerCase())));
    if (hasHighRisk)
        score -= 5;
    // Check for responsible sourcing mentions
    const sourcingClaims = claims.filter(c => c.type === 'sourcing' || c.type === 'policy');
    const hasResponsibleSourcing = sourcingClaims.some(c => RESPONSIBLE_SOURCING_KEYWORDS.some(kw => c.text.toLowerCase().includes(kw)));
    if (hasResponsibleSourcing)
        score += 10;
...
function calculateTransparencyScore(claims) {
    let score = 10; // Start at low-mid
    // Check for transparency indicators
    const allText = claims.map(c => c.text.toLowerCase()).join(' ');
    const transparencyMatches = TRANSPARENCY_KEYWORDS.filter(kw => allText.includes(kw.toLowerCase()));
...
function calculateRepairabilityScore(product, claims) {
    ...
    if (recyclingClaims.length > 0)
        score += 5;
    // Check for take-back programs
    const hasTakeBack = claims.some(c => c.text.toLowerCase().includes('take-back') ||
        c.text.toLowerCase().includes('recycling program') ||
        c.text.toLowerCase().includes('trade-in'));
    if (hasTakeBack)
        score += 5;
    // Check for recycled materials
    const hasRecycled = claims.some(c => c.text.toLowerCase().includes('recycled'));
    if (hasRecycled)
        score += 5;
    return Math.max(0, Math.min(30, score));
}
router.post('/', async (req, res) => {
    try {
        // Validate request
        const parseResult = schemas_js_1.EcoScoreRequestSchema.safeParse(req.body);
        ...
        const { summary, rationaleBullets } = await (0, vertexai_js_1.generateEcoScoreExplanation)(detectedProduct, { total, sourcing, transparency, repairability, gemmaTips: [] }, evidencePack.claims);
        // Get Gemma tips
        const gemmaTips = await (0, vertexai_js_1.generateGemmaTips)(detectedProduct, detectedProduct.materialsUsed);
        const ecoScore = {
            total,
            sourcing,
            transparency,
...
//# sourceMappingURL=ecoscore.js.map